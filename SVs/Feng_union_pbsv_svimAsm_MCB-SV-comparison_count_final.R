...
CSV = "/data5/sv/sam_SV/Duroc_MCB/union_pbsv_svim-asm/Yuc104F_MCB_to_Ssc11.1_pbsv_svimAsm_Survivor1100_union_VCF_data.csv"
...
SV_vcf_data <- fread(CSV) %>%
  dplyr::select(CHROM, POS, ID, REF, ALT, FORMAT, Yuc104F, `GC64-23_008_MCB_FSDC`)
names(SV_vcf_data)[names(SV_vcf_data) == "GC64-23_008_MCB_FSDC"] <- "MCB" # rename to MCB for simplicity
...
SMCB_expanded <- expand_sample_column(SV_vcf_data$`MCB`, format_fields, "MCB")
SV_vcf_wide <- cbind(SV_vcf_data, yuc_expanded, SMCB_expanded)  %>%   dplyr::select(-Yuc104F, -`MCB`,-FORMAT)
...
support_MCB <- parse_supp_vec(SVs$`MCB_PSV`, caller_names, "MCB")

SVs <- cbind(SVs, support_Yuc, support_MCB)
# remove chrY and MT variants
SVs <- SVs %>% filter(CHROM != "Y") %>%  filter(CHROM != "MT")

# Set variable for whether the SV was only supported in MCB, as in was wild-type or unknown for Yuc104F and non-WT for MCB
WT_GT_values <- c("./.", "0/0", "0/.", "./0") # WT values

# Make as variable to keep info in main data frame
SVs$`MCB_specific` <- FALSE 
SVs$`MCB_specific`[SVs$Yuc104F_GT %in% WT_GT_values & !(SVs$`MCB_GT` %in%  WT_GT_values)] <- TRUE 
...
#lowMQ10_bed_MCB <- fread(file = "../projects/egenesis/sammodlin/egenesis/sammodlin/sammodlin-sandbox/manual_curation/guides/MCB_HiFito_Ssc11.1.lowMQ10.bed", col.names = c("CHROM", "START", "END", "threshold"))
#lowMQ10_bed <- rbind(lowMQ10_bed_MCB, lowMQ10_bed_Yuc104F) 
...
introns_pc_all <- GenomicRanges::reduce(unlist(introns_pc, use.names = FALSE))
...
fwrite(SVs, "./MCB/Feng_Processed_union_pbsv_svimAsm_Yuc104F_var_MCB_SVs.csv")
...
SVs <- fread("./MCB/Feng_Processed_union_pbsv_svimAsm_Yuc104F_var_MCB_SVs.csv") 
...
filters <- list(	
  ...
  "MCB-specific" = expr(`MCB_specific`),					# MCB-specific
  "MCB-specific + region" = expr(`MCB_specific` & !in_filt_feature), 	# + sequence pattern
  "MCB-specific + region + size (20–1000 bp)" =				# + SV Length (20-1000 bp)
    expr(`MCB_specific` & !in_filt_feature & ((abs(SVLEN) >= 20 & abs(SVLEN) <= 1000) | SVTYPE == "TRA")),
  "MCB-specific + region + size (20–500 bp)" = 				# + SV Length (20-500 bp)
    expr(`MCB_specific` & !in_filt_feature & ((abs(SVLEN) >= 20 & abs(SVLEN) <= 500)  | SVTYPE == "TRA")),
  "MCB-specific + region + PC (any size)" =   # + exon/intron
    expr(`MCB_specific` & !in_filt_feature & in_exonic_or_intronic_pc),
  "MCB-specific + region + size (20–1000 bp) + PC" = 			# + exon/intron (20-1000 bp)
    expr(`MCB_specific` & !in_filt_feature & ((abs(SVLEN) >= 20 & abs(SVLEN) <= 1000) | SVTYPE == "TRA") & in_exonic_or_intronic_pc),
  "MCB-specific + region + size (20–500 bp) + PC" =			# + exon/intron (20-500 bp)
    expr(`MCB_specific` & !in_filt_feature & ((abs(SVLEN) >= 20 & abs(SVLEN) <= 500)  | SVTYPE == "TRA") & in_exonic_or_intronic_pc)
)
...
fwrite(sv_summary, "./MCB/Feng-MCB-unionof2-SV-filtering-summary.csv")
...
fwrite(VCF_full_filt_no_size_1_2_small, "./MCB/Feng_MCB_unionOf2_full-filtered_no_size.csv")

# For manual curation excel
VCF_full_filt_no_size_1_2_small_csv <- VCF_full_filt_no_size_1_2 %>% as.data.table() %>% dplyr::select(CHROM, POS, ID, REF, ALT, SVLEN, SVTYPE, QUAL, CHR2, STRANDS, `MCB_GT`, Yuc104F_GT, `MCB_pbsv`, Yuc104F_pbsv, `MCB_svim-asm`, `Yuc104F_svim-asm`, `MCB_LN`, Yuc104F_LN, `MCB_QV`, Yuc104F_QV, `MCB_CO`, Yuc104F_CO, `MCB_ID`, Yuc104F_ID,  `MCB_TY`, Yuc104F_TY, `MCB_AAL`, Yuc104F_AAL )
fwrite(VCF_full_filt_no_size_1_2_small_csv, "./MCB/Feng_MCB_unionOf2_full-filtered_no_size_man_cur.csv")

######################################################################
### Distribution of feature overlaps with MCB-specific SVs ###
######################################################################
summary_SVs_feat_overlap_MCB_spec_all <- SVs %>% filter(`MCB_specific` == TRUE) %>%
  group_by(SVTYPE) %>%
  summarise(across(all_of(c(feature_cols, "in_any_feature")), ~ sum(.x, na.rm = TRUE), .names = "{.col}")) %>%
  ungroup()
summary_SVs_feat_overlap_MCB_spec_all

summary_SVs_feat_overlap_MCB_spec <- SVs %>% filter(`MCB_specific` == TRUE) %>%
  group_by(SVTYPE) %>%
  summarise(across(all_of(c(filt_feature_cols, "in_filt_feature")), ~ sum(.x, na.rm = TRUE), .names = "{.col}")) %>%
  ungroup()
summary_SVs_feat_overlap_MCB_spec

summary_SVs_feat_overlap_MCB_spec_sizefilt_all <- SVs %>% filter(`MCB_specific` == TRUE) %>%
  filter((abs(SVLEN)<=1000 & abs(SVLEN)>=20) | SVTYPE == "TRA") %>%
  group_by(SVTYPE) %>%
  summarise(across(all_of(c(feature_cols, "in_any_feature")), ~ sum(.x, na.rm = TRUE), .names = "{.col}")) %>%
  ungroup()
summary_SVs_feat_overlap_MCB_spec_sizefilt_all

summary_SVs_feat_overlap_MCB_spec_sizefilt <- SVs %>% filter(`MCB_specific` == TRUE) %>%
  filter((abs(SVLEN)<=1000 & abs(SVLEN)>=20) | SVTYPE == "TRA") %>%
  group_by(SVTYPE) %>%
  summarise(across(all_of(c(filt_feature_cols, "in_filt_feature")), ~ sum(.x, na.rm = TRUE), .names = "{.col}")) %>%
  ungroup()
summary_SVs_feat_overlap_MCB_spec_sizefilt
...
p2 <- SVs %>% filter(`MCB_specific` == TRUE) %>% ggplot(aes(x = log10(abs(SVLEN)+1), fill = SVTYPE)) +
  geom_histogram(bins = 50, alpha = 0.9, position = "stack") +
...
p3 <- SVs %>% filter(`MCB_specific` == TRUE) %>%
  filter(in_filt_feature == FALSE) %>%
...
p4 <- SVs %>% filter(`MCB_specific` == TRUE) %>%
  filter(in_filt_feature == FALSE) %>%
  filter((abs(SVLEN)<=1000 & abs(SVLEN)>=20) | SVTYPE == "TRA") %>%
...
p5 <- SVs %>% filter(`MCB_specific` == TRUE) %>%
  filter(in_filt_feature == FALSE) %>%
...
p6 <- SVs %>% filter(`MCB_specific` == TRUE) %>%
  filter(in_filt_feature == FALSE) %>%
  filter(in_exonic_or_intronic_pc == TRUE) %>%
  ggplot(aes(x = log10(abs(SVLEN)+1), fill = SVTYPE)) +
  geom_histogram(bins = 50, alpha = 0.9, position = "stack") +
...
p7 <- SVs %>% filter(`MCB_specific` == TRUE) %>%
  filter(in_filt_feature == FALSE) %>%
  filter((abs(SVLEN)<=500 & abs(SVLEN)>=20) | SVTYPE == "TRA") %>%
  filter(in_exonic_or_intronic_pc == TRUE) %>%
  ggplot(aes(x = abs(SVLEN), fill = SVTYPE)) +
...
p8 <- SVs %>% filter(`MCB_specific` == TRUE) %>%
  filter(in_filt_feature == FALSE) %>%
  filter((abs(SVLEN)<=1000 & abs(SVLEN)>=20) | SVTYPE == "TRA") %>%
  filter(in_exonic_or_intronic_pc == TRUE) %>%
  ggplot(aes(x = abs(SVLEN), fill = SVTYPE)) +
